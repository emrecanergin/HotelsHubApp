...
namespace HotelsHubApp.Core.Utilities.HttpRequest.HotelbedsAPI
{
    ...
    public class HttpRequestService<T> : IHttpRequestService<T> where T : class
    {
        ...
        public async Task<T> PostRequestAsync(object requestBody,string target) 
        {
            var client = _httpClientFactory.CreateClient("CustomRequest");
        ...
        public async Task<T> GetRequestAsync(string target)
        {
            var client = _httpClientFactory.CreateClient("CustomRequest");
